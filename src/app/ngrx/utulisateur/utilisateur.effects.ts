...
export class UtilisateurEffects {
  ...
  getAllProprietairesEffect: Observable<Action> = createEffect(() =>
    ...
    this.effectActions.pipe(
      ...
      mergeMap(() => {
        return this.apiService.getAllUtilisateursByOrder().pipe(
          map(
            (proprietaires) =>
              new GetAllUtilisateursActionsSuccess(proprietaires)
          ),
          catchError((err) => of(new GetAllUtilisateursActionsError(err.message)))
        );
      }),tap((proprio)=>{
        if(proprio.payload.indexOf('Error') < 0){
          this.sendErrorNotification(NotificationType.SUCCESS,'La liste des Utilisateurs a été chargées avec succès')
        }else{
          this.sendErrorNotification(NotificationType.ERROR,'Une erreur a été rencontré!')
        }
      })
    ...
    this.effectActions.pipe(
      ...
      mergeMap((action:UtilisateurActions) => {
        return this.apiService.saveUtilisateur(action.payload).pipe(
          map(
            (utilisateur) =>
              new SaveUserActionsSuccess(utilisateur)
          ),
      ...
      tap((utilisateur) => {
        ...
        if(utilisateur.type.indexOf('Error') < 0){
          this.sendErrorNotification(NotificationType.SUCCESS,'La liste des locataires a été chargées avec succès')
        }else{
          this.sendErrorNotification(NotificationType.ERROR,'Une erreur a été rencontré')
        }
      })
