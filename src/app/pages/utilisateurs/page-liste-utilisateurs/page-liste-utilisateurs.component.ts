  ...
  ngOnInit(): void {
      ...
      this.subscriptions.push(
        this.userService.addUser(this.editUser!).subscribe(
          (response: any) => {
            ...
            //userForm.reset();
            this.sendErrorNotification(NotificationType.SUCCESS, `${response.firstName} ${response.lastName} updated successfully`);
          },
          (errorResponse: HttpErrorResponse) => {
            this.sendErrorNotification(NotificationType.ERROR, errorResponse.error.message);
            this.profileImage = null;
    ...
    public searchUsers(searchTerm: string): void {
      ...
      for (const user of this.userService.getUsersFromLocalCache()) {
        if (user.nom!.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1 ||
            user.prenom!.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1 ||
            user.username!.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1 ||
            user.utilisateurIdApp!.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1) {
            results.push(user);
        }
      }
      this.users = results;
